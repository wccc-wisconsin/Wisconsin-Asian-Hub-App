...
export default function DonateForm({ onClose }: DonateFormProps) {
  ...
  const [donorPhone, setPhone]  = useState('')
  const [submitting, setSubmitting] = useState(false)
  const [error, setError]           = useState('')

  const isValid = title && description && category && city && donorName && donorEmail && donorPhone
  ...
  async function handleSubmit() {
    ...
    setError('')
    try {
      await submitDonation({ title, description, category, city, donorName, donorEmail, donorPhone })
      onClose()
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        ...
        <div>
          ...
          <textarea placeholder="Condition, dimensions, any relevant details..."
        ...
        <div className="rounded-xl p-4 space-y-3" style={{
          ...
          <p className="text-xs font-semibold" style={{ color: 'var(--color-red)' }}>📬 Your Contact Info</p>
          <p className="text-xs" style={{ color: 'var(--color-muted)' }}>
            Shared with matched member to arrange pickup
          </p>
          <input type="text" placeholder="Your name *"
            value={donorName} onChange={e => setName(e.target.value)}
            className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
          <input type="email" placeholder="Email *"
            value={donorEmail} onChange={e => setEmail(e.target.value)}
            className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
          <input type="tel" placeholder="Phone *"
            value={donorPhone} onChange={e => setPhone(e.target.value)}
            className="w-full px-4 py-2.5 rounded-lg text-sm outline-none" style={inputStyle} />
        </div>
